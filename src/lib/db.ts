...
import { Project, newUserAddresses } from '../types';
...
export const insertAttestation = async (attestation: NewAttestation) => {
  try {
    ...
    const result = await db.insert(contributionattestations).values(attestation).returning();
...
export const getAttestationsByUserId = async (userFid: string) => {
  try {
    ...
    console.error(`Error retrieving attestations for user '${userFid}':`, error);
    throw error;
  }
};
...
export const getUserProjectAttestations = async (userProjectNames: string[]) => {
  try {
    ...
    console.error(`Error retrieving projects for user '${userProjectNames.join(',')}':`, error);
    throw error;
  }
};
...
export const insertUserAddress = async (addresses: newUserAddresses[]) => {
  try {
    const userAddressInsert = await db.insert(user_addresses).values(addresses).returning();
...
export const updateEthereumAddressStatus = async (
  ...
  try {
    const updated = await db
      ...
      .set(statusUpdates)
      .where(eq(user_addresses.userfid, userFid) && eq(user_addresses.ethaddress, ethAddress))
      .returning();
    return updated;
  } catch (error) {
